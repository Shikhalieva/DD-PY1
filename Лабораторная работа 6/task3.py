...
def do_replacing(item: str, old_list: list, new_list: list):
    for index in range(len(old_list)):
        item = item.replace(old_list[index], new_list[index])
    return item


def to_csv_file(filename, headers, rows, delimiter=",", new_line="\n"):
    file = open(filename, "w")
    file.write(do_replacing(str(headers) + new_line, ["[", "]", "'", " "], [""]*4))
    for item in rows:
        file.write(do_replacing(str(item) + new_line, ["[", "]", "'", " "], [""]*4))
    file.close()
